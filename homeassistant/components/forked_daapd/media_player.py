...
class ForkedDaapdMaster(MediaPlayerEntity):
    ...
    async def async_play_media(self, media_type, media_id, **kwargs):
        """Play media."""
        if media_type == MEDIA_TYPE_MUSIC and media_id.find("/api/tts_proxy/") != -1:
            return await self._async_play_tts(media_id)
        if media_type in CAN_PLAY_TYPE:
            media_id = media_id.split("&")[1]
            return await self._api.add_to_queue(
                uris=media_id,
                playback="start",
                clear=(not kwargs.get(ATTR_MEDIA_ENQUEUE)),
            )
        _LOGGER.debug("Media type '%s' not supported", media_type)

    async def _async_play_tts(self, media_id):
        """Play a URI."""
    ...
    async def async_browse_media(self, media_content_type=None, media_content_id=None):
        ...
        return await build_item_response(self._api, payload)
